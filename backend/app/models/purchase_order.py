...
class PurchaseOrderSequence(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "sequence_year",
            "sequence_month",
            name="uq_po_sequence_prefix_year_month",
        ),
    )
...
class PurchaseOrder(Base):
    ...
    vendor_eta: Mapped[date | None] = mapped_column(Date, nullable=True)
    vendor_response_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    vendor_rejection_reason: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    ...
    attention_items: Mapped[list["PurchaseOrderAttention"]] = relationship(
        back_populates="purchase_order",
        cascade="all, delete-orphan",
        order_by="PurchaseOrderAttention.created_at.desc()",
    )


class PurchaseOrderAttention(Base):
    __tablename__ = "purchase_order_attention"

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    purchase_order_id: Mapped[str] = mapped_column(
        ForeignKey("purchase_orders.id", ondelete="CASCADE"), index=True
    )
    initiated_by_side: Mapped[str] = mapped_column(String(16), index=True)
    action_type: Mapped[str] = mapped_column(String(32), index=True)
    status: Mapped[str] = mapped_column(String(16), default="pending", index=True)
    payload: Mapped[dict] = mapped_column(JSON, default=dict)
    reason: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    response_note: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    created_by: Mapped[str] = mapped_column(String(320))
    responded_by: Mapped[str | None] = mapped_column(String(320), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    responded_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)

    purchase_order: Mapped["PurchaseOrder"] = relationship(back_populates="attention_items")
...
class PurchaseOrderTransmission(Base):
    ...
    artifact_id: Mapped[str] = mapped_column(ForeignKey("purchase_order_artifacts.id"), unique=True)
