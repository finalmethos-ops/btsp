  ...
  return (
    <main className="mx-auto max-w-6xl p-8">
      <header className="mb-8">
        <a className="text-sm text-slate-600" href="/">← Back to dashboard</a>
        <h1 className="mt-4 text-3xl font-bold">Administration</h1>
        <p className="mt-2 text-slate-600">Manage BTSP users, store data, configuration, and audit records.</p>
      </header>
      ...
      <div className="grid gap-6 md:grid-cols-[220px_1fr]">
        <nav className="rounded-lg bg-white p-4 shadow">
          ...
          <div className="flex flex-col gap-2">
            {navigation.map((item) => (
              <a className="rounded px-3 py-2 text-sm hover:bg-slate-100" href={item.href} key={item.href}>
                {item.label}
              </a>
            ))}
          </div>
